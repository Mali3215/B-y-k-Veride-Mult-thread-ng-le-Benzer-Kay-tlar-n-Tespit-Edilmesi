...
public class StopWordRemoval {
    String[] stopWords = { "a", "about", "above", "across", "after", "again",
            ...
            "fact", "facts","false", "far", "felt", "few", "find", "finds", "first",
            ...
            "your", "yours", "z" };
        public String remoweStopWords(String row){

            String[] words=row.split(" ");
            for(int i=0;i<words.length;i++){
                for(int j=0;j<stopWords.length;j++){

                    if (Objects.equals(words[i], stopWords[j])) {
                        words[i] = "";
            ...
            row= Arrays.toString(words);
            row=RemovePunctuation.removePunc(row);
